  ...
  <div style="width:100%;min-height: calc(100vh - 84px);background-color:#EEEEEE">
      ...
      <el-main width="78%" style="padding:0" class="right-box">
        ...
        <div style="width:100%;">
          ...
          <el-button type="info" class="filter-item" style="margin: 10px;" @click.prevent="updateChildNode"
            v-hasPermi="['system:node:remove']">修改节点</el-button>
        ...
        <div class="image_box img-list" v-else>
          <el-card class="image_item item" v-for="(item, index) in imageSrcList" :key="item.pictureId">
            <div class="wrapper">
              <div class="imgBox">
                <el-image :src="item.pictureUrl" :preview-src-list="imageSrcList.map(item => item.pictureUrl)"
                  :initial-index="index"
                  style="min-width: 168px;min-height:168px;text-align: center;line-height: 168px;font-size: 40px;"
                  fit="fill">
    ...
    <el-dialog title="添加图片" v-model="imageDialog" center draggable width="50%">
      <el-upload v-model:file-list="fileList" class="upload-demo" ref="upload" accept=".jpeg,.jpg,.png,.gif,.bmp,.webp"
        ...
        :on-error="uploadImageError" :on-success="uploadImageSuccess" list-type="picture">
...
const getTreeList = () => {
  getTree(props.treeType, 0, 1).then(res => {
    ...
    // nextTick(async () => {
    //   const curNode = tree.value.getCurrentNode();
    //   imageSrcList.value = await getImagesBynodeId(curNode.treeId ? curNode.treeId : );
    // });
  });
};
